...
class NonMaxSuppression:
    ...
    def version_11(cls, ctx, node, **kwargs):
        ...
        dtypes = [ctx.get_dtype(node.output[0])]
        shapes = [ctx.get_shape(node.output[0])]
        dtypes_1 = [ctx.get_dtype(node.output[1])]
        shapes_1 = [ctx.get_shape(node.output[1])]
        ...
        slice_op = GraphBuilder(ctx).make_slice({"data": new_nonmaxsurppress,
                                                 "axes": [1], "ends": [3], "starts": [2]})
        squeeze_op = ctx.make_node("Squeeze", [slice_op], attr={"axes": [1]})
        if node.attr["pad_to_max_output_size"] == 0:
            ctx.make_node("Cast", inputs=squeeze_op.output, attr={"to": onnx_pb.TensorProto.INT32},
                          name=node.name, outputs=node.output, dtypes=dtypes, shapes=shapes)
        else:
            ...
            ctx.make_node("Cast", inputs=pad_op.output, name="cast_A", attr={"to": onnx_pb.TensorProto.INT32}, outputs=[node.output[0]], shapes=shapes)
            reduce_op = ctx.make_node("ReduceSum", inputs=shape_op.output, attr={"axes":[0], "keepdims":0})
            ctx.make_node("Cast", inputs=[reduce_op.output[0]], name="cast_B", attr={"to": onnx_pb.TensorProto.INT32}, outputs=[node.output[1]], shapes=shapes_1)
        del node.attr["pad_to_max_output_size"]
